    ...
    while True:
        ...
        if frame_count % 10 == 0:
            ...
            for i in range(scores_np.shape[0]):
                class_id = scores_np[i].argmax()
                if CLASSES[class_id] == "no_helmet":
                    has_no_helmet = True
                    break

            if has_no_helmet:
                save_path = os.path.join(args.output_dir, f"alert_frame_{frame_count}.jpg")

                print("图片绝对保存路径：", os.path.abspath(save_path))  # <-- 必须打印这个！

                ok = cv2.imwrite(save_path, frame)
                print("保存结果：", ok)

                if ok:
                    print(f"⚠️ 警报提醒！检测到未佩戴安全帽，已保存图片：{os.path.abspath(save_path)}")
                else:
                    print(f"❌ 保存失败：{os.path.abspath(save_path)}")
