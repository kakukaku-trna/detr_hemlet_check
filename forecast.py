...
def main(args):
    ...
    image_file_path = os.listdir("inference_demo/detect_demo/tu")
    image_set = []

    for image_item in image_file_path:
        print("inference_image:", image_item)
        image_path = os.path.join("inference_demo/detect_demo/tu", image_item)
        ...
        # ================= 打印本次图片的推理结果 =================
        print(f"【{image_item}】检测到 {len(boxes)} 个目标")
        for i in range(boxes.shape[0]):
            cls_id = int(scores[i].argmax())
            label = CLASSES[cls_id]
            conf = scores[i].max()
            x1, y1, x2, y2 = boxes[i]
            print(f"  {i + 1}. {label}  confidence={conf:.3f}  box=({x1:.1f},{y1:.1f},{x2:.1f},{y2:.1f})")
        # ==========================================================
        for i in range(boxes.shape[0]):
        ...
        image.save(os.path.join(args.output_dir, f"test.jpg"))
        save_path = os.path.join(args.output_dir, f"test.jpg")
        image.save(save_path)
        print("结果图已保存至：", save_path)  # <-- 只加这一行
